...
function _getFilteredBooks(books, filterBy) {
    ...
    return books.filter(book => {
        let { amount } = book.listPrice

        return book.title.includes(name) && amount >= minPrice && amount <= maxPrice
    })
...
function getBooksFromGoogle(searchString) {

    const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchString}`
    if (!searchString) return Promise.resolve([])
    return axios.get(url)
        .then(res => res.data)
        .then(searchRes => {
            const firstFiveRes = searchRes.items.splice(0, 5)
            return firstFiveRes
        })
